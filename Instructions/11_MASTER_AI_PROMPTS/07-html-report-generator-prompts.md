# 07 — HTML Report Generator Prompts (PowerShell → Rich HTML)

> **What this is**: AI prompts that generate production-quality, rich HTML reports from PowerShell data. Turn raw AD diagnostic output into executive-ready dashboards and engineer-ready reports — all read-only, all safe.

---

## PROMPT 1: Generate PowerShell → HTML Report Script

```
Generate a complete PowerShell script that collects Active Directory health data and produces a rich, professional HTML report. The report must be:

REQUIREMENTS:
- Read-only (zero changes to AD)
- Self-contained (one .ps1 file outputs one .html file)
- Professional appearance (CSS styled, not plain text)
- Executive-readable summary at top
- Technical details expandable below
- Color-coded status indicators (Green/Yellow/Red)
- Timestamp, domain, and collector information in header
- Safe to run on production DCs without risk
- Works with PowerShell 5.1 and AD module

REPORT SECTIONS:
1. Executive Dashboard (health score, top issues, quick stats)
2. Domain Controller Status (all DCs, services, replication)
3. Replication Health (partner status, latency, errors)
4. DNS Health (SRV records, zone status)
5. Account Health (locked accounts, expiring passwords, stale accounts)
6. Group Policy (recently modified GPOs, GPO count)
7. Security Indicators (failed logons, privilege use, recent changes)
8. Recommendations (auto-generated based on findings)

OUTPUT: Complete, runnable PowerShell script with embedded CSS.

ENVIRONMENT:
[Domain name, DC count, special requirements]
```

---

## MASTER HTML REPORT TEMPLATE (PowerShell)

Save as `Invoke-ADHealthReport.ps1` — complete, production-ready:
...
## PROMPT 2: Executive Security Posture Report

```
Generate a PowerShell script that produces a one-page executive HTML dashboard showing our Active Directory security posture. The report should:

- Be readable by a CISO or CTO in under 2 minutes
- Show RAG (Red/Amber/Green) status for each security domain
- Include trend arrows (improving/worsening)
- Have a risk score (0-100)
- Show top 3 risks with business impact
- Include recommended next actions with effort/impact ratings
- Use professional enterprise styling

Security domains to cover:
1. Privileged Access Management
2. Account Security (lockouts, stale accounts, password hygiene)
3. Replication & Infrastructure Health
4. Hybrid Identity Posture
5. Patch/OS Currency
6. Certificate Health (AD CS)
7. Monitoring Coverage

Output: Complete PowerShell script with embedded CSS.
```

---

## PROMPT 3: Incident Post-Mortem HTML Report

```
Generate a professional HTML post-mortem/post-incident report for an Active Directory incident. The report should be suitable for:
- Executive briefing
- Lessons learned archive
- Regulatory/compliance documentation

Sections to include:
1. Executive Summary (1 page worth)
2. Incident Timeline (visual)
3. Root Cause Analysis (5 Whys)
4. Impact Assessment (users, services, revenue, SLA)
...
INPUT DATA:
[Paste your incident summary here — dates, symptoms, cause, resolution]

Generate both:
A. The HTML report structure with CSS
B. A PowerShell script to produce it programmatically
```
